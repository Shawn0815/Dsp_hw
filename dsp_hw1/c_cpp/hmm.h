...
typedef struct{
    char *model_name;
    int state_num;					//number of hmm state
    int observ_num;					//number of observation
    double initial[MAX_STATE];			//initial prob.
    double transition[MAX_STATE][MAX_STATE];	//transition prob matrix.
    double observation[MAX_OBSERV][MAX_STATE];	//observation prob matrix.
} HMM;
...
static void loadHMM( HMM *hmm, const char *filename )
{
    ...
    while( fscanf( fp, "%s", token ) > 0 )
    {
        ...
        // Get the state num from init.txt
        ...
        // String Compare function: strcmp, if true return 0
        // fscanf: read values from txt file
        // Read transition matrix from the txt file
